...
func TestBuildAuditRunner_GatedOnConsumingLoop(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			r, err := buildAuditRunner(auditor.NewMemQueue(), auditor.NewMemStatusStore(), svc, pool, local.New(),
	...
	pool := memstore.NewPool()
	svc := vcresolver.New(memstore.NewStore(), pool)
	if r, err := buildAuditRunner(auditor.NewMemQueue(), auditor.NewMemStatusStore(), svc, pool, local.New(), auditCfg(nil)); err != nil || r != nil {
...
func TestAuditRunner_Integration_HoleLivenessFinalize(t *testing.T) {
	...
	r, err := buildAuditRunner(queue, status, svc, pool, local.New(), auditCfg([]pipelineconfig.LoopConfig{{Role: pipelineconfig.RoleSink}}))
	if err != nil || r == nil {
