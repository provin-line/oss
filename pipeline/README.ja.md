...
- 任意のクレデンシャルはソースコミットメントを運んで**よい**（audit-reachable conformance class — [originsource/README.md](originsource/README.md) 参照）: 消費した適合 source 全集合への監査属性であり、親リンクではない。`previousCredential` と直交し、chain-preserving ではトリガーの先行イベントも commit 対象に含む（全消費分セマンティクス）。トリガー意味論と線形性には影響しない。
