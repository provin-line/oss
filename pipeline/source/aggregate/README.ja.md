# aggregate — Source Process メカニクス: プール + ウィンドウ
...
- **ステートフル**なパイプラインワークロード（プール、ウィンドウ、ジョイン）が属する場所はここである — Chained Process のステートレス性は定義的性質であるため、集約はそちらに移行できない。
- 実行は timer / window 満了でトリガーされる — 単一の前イベントによってではない — ため、出力は `transformationClaim: "provin:aggregate"` の FirstDrop になる（トリガー規則）。たまたま pending 1 件だけをたたみ込んだ実行も FirstDrop（batch-of-1 規則）。
