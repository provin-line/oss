# source — Source Process: FirstDrop Issuance

The Source Process type. Its **only definitional property** is emitting a new
...
The chain is strictly linear: a Source Process's FirstDrop carries **no upstream
...
[pipeline/README.md](../README.md)): Source Process mechanics are exactly the
...
|---|---|---|---|
| `ingest/` | external push / file / poll / non-conformant credential arrival (boundary translation) | stateless ingestion | `apipush/` reference implementation |
| `aggregate/` | timer / window over pooled Pipeline-conformant inputs | pool + windowing (stateful) | conventions only |

Enrichment is **not** a Source Process mechanics: it is a chain-preserving
Chained Process step pattern (see
[../chained/README.md](../chained/README.md)).

Mechanics are conventions, not Type Contract distinctions — at the protocol level
there is exactly one Source Process type.
