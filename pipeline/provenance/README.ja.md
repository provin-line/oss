...
- `Provider` — `Sign(ctx, payload, inputHash, outputHash) (*Credential, error)`；プロセスごとのチェーン状態（`previousCredential` リンク）を管理し、audit-reachable conformance class で deploy された境界の場合（config 駆動、Source Process に限らない — chain-preserving は先行イベントを含む全消費分に commit する）はソースコミットメント（`vc.SourceCommitment` — [../source/README.md](../source/README.md) 参照）も管理する。
