...
func TestParse_Owner(t *testing.T) {
	d, err := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme")
	if err != nil {
...
func TestParse_Pipeline(t *testing.T) {
	d, err := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1")
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	if !d.IsPipeline() {
...
func TestParse_Process(t *testing.T) {
	d, err := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1:process:proc1")
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	if !d.IsProcess() {
...
func TestParse_Rejects(t *testing.T) {
	cases := []string{
		...
		"did:dplaax:poc.dplaax.io",           // missing accountType+accountId
		"did:dplaax:poc.dplaax.io:org",       // missing accountId
		"did:dplaax:poc.dplaax.io:org:",      // empty accountId segment
		"did:dplaax:poc.dplaax.io:org:acme:", // trailing empty resource segment
		"did:dplaax:poc.dplaax.io:org:ac/me", // slash not in safe-segment set
		"did:dplaax:poc.dplaax.io:org:..",    // all-dots segment (traversal)
		"did:dplaaxx:poc.dplaax.io:org:acme", // method is not exactly dplaax
		"did:dplaax:poc.dplaax.io:org:ac me", // space
	}
	for _, s := range cases {
...
func TestString_RoundTrip(t *testing.T) {
	for _, s := range []string{
		"did:dplaax:poc.dplaax.io:org:acme",
		"did:dplaax:poc.dplaax.io:org:acme:pipeline:p1",
		"did:dplaax:poc.dplaax.io:org:acme:pipeline:p1:process:proc1",
	} {
...
func TestTruncation(t *testing.T) {
	d, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1:process:proc1")

	owner := d.OwnerDID()
	if !owner.IsOwner() || owner.String() != "did:dplaax:poc.dplaax.io:org:acme" {
		t.Errorf("OwnerDID()=%q", owner.String())
	}
	pipe := d.PipelineDID()
	if pipe == nil || !pipe.IsPipeline() || pipe.String() != "did:dplaax:poc.dplaax.io:org:acme:pipeline:p1" {
	...
	if pipe.String() != "did:dplaax:poc.dplaax.io:org:acme:pipeline:p1" {
		t.Error("PipelineDID() aliased the original ResourcePath — defensive copy required")
	}
}

func TestIsSafeSegment(t *testing.T) {
	safe := []string{"acme", "poc.dplaax.io", "p1", "proc_1", "a-b", "x.y.z", "1"}
	for _, s := range safe {
...
func TestValidateDID(t *testing.T) {
	good, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1")
	if err := dplaax.ValidateDID(good); err != nil {
		t.Errorf("ValidateDID(good): %v", err)
	}

	// Unsupported account type.
	badType, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:person:acme")
	...
	unknown, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:widget:w1")
...
func TestRequireKnownPattern(t *testing.T) {
	...
	bad, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:widget:w1")
	...
	half, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1:process")
...
func TestRequireOwner(t *testing.T) {
	owner, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme")
	if err := dplaax.RequireOwner(owner); err != nil {
		t.Errorf("RequireOwner(owner): %v", err)
	}
	proc, _ := dplaax.Parse("did:dplaax:poc.dplaax.io:org:acme:pipeline:p1:process:proc1")
