...
// Store is the file-backed vcresolver.Store: one canonical-JSON file per
// credential, named by its content address.
type Store struct {
	mu  sync.RWMutex
	dir string
}
...
// NewStore opens (creating if needed) the credential store rooted at dir. An
// uncreatable root is an error — a node that cannot persist evidence must not
// pretend to (boot fails closed).
...
// Put persists cred at hash (overwriting is harmless — content-addressed, so
// the same hash carries the same body).
func (s *Store) Put(hash string, cred *vc.PipelinePassCredential) error {
	path, err := entryFile(s.dir, hash)
	if err != nil {
		return err
	}
	raw, err := cred.MarshalJSON()
	if err != nil {
		return fmt.Errorf("filestore: marshal credential %s: %w", hash, err)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if err := writeAtomic(path, raw); err != nil {
		return fmt.Errorf("filestore: write credential %s: %w", hash, err)
	}
	return nil
}

// Get returns the credential at hash, vcresolver.ErrNotFound when absent, or
// a distinct error for a damaged entry. The decoded body's content address is
// recomputed and compared to the key: a tampered-but-well-formed file must
// not be served (and later audited) under the original address.
func (s *Store) Get(hash string) (*vc.PipelinePassCredential, error) {
	...
	if errors.Is(err, fs.ErrNotExist) {
		...
		// unmounted root must not read as "credential absent" (heads would be
		// dropped and peers 404'd on what is a storage failure).
		if _, statErr := os.Stat(s.dir); statErr != nil {
			return nil, fmt.Errorf("filestore: credential store root %s unavailable: %w", s.dir, statErr)
		}
		return nil, vcresolver.ErrNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("filestore: read credential %s: %w", hash, err)
	}
	...
	if err := json.Unmarshal(raw, &cred); err != nil {
		return nil, fmt.Errorf("filestore: damaged credential entry %s: %w", hash, err)
	}
	got, err := cred.Hash()
	if err != nil {
		return nil, fmt.Errorf("filestore: damaged credential entry %s: %w", hash, err)
	...
	if got != hash {
		return nil, fmt.Errorf("filestore: damaged credential entry %s: stored body hashes to %s (tampered or misfiled)", hash, got)
	}
	return &cred, nil
}
