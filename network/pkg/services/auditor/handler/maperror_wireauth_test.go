...
func TestMapError_WireauthDelegation(t *testing.T) {
	if got := connect.CodeOf(mapError(fmt.Errorf("auditor: verify: %w", wireauth.ErrBeforeEpoch))); got != connect.CodeUnavailable {
	...
	if got := connect.CodeOf(mapError(auditor.ErrHeadNotAdmitted)); got != connect.CodeFailedPrecondition {
