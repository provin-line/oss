...
// never a false Verified. In-memory for the PoC (lost on restart, like the status store).
type ReceiptStore interface {
	...
	// Get returns the consumed source content addresses for headHash and whether a receipt
	// exists. Absence is the coverage gate: no receipt → the head is audited linear-only.
...
// Get returns the consumed source content addresses for headHash and whether one exists.
func (s *MemReceiptStore) Get(headHash string) ([]string, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	c, ok := s.m[headHash]
	return c, ok
