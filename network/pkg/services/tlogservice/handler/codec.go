...
// RFC 3339 UTC second-precision string. Mapped to InvalidArgument.
// Reproduced (not shared) from auditor/handler, payloadresolver/handler, and
// chainmanager/handler's own copies — the codebase's convention for keeping
// L2 surfaces decoupled.
var errMalformedIssuedAt = errors.New("tlogservice: malformed issued_at")
...
// parseIssuedAt strictly decodes the wire issued_at string: it accepts ONLY
// the canonical RFC 3339 UTC second-precision form (the exact form wireauth
// signs over) and rejects fractional seconds, a non-Z offset, or any
// non-canonical string BEFORE the proof reaches wireauth.Verify.
